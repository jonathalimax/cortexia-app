...
struct OllamaAIClient {
	@Dependency(\.logger) private var logger

	private var request: (_ endpoint: Endpoint, _ body: Data?) async throws -> Data

	/// Makes a request to the Ollama API and decodes the response.
	///
	/// - Parameters:
	...
	static var live: Self {
		...
		return OllamaAIClient(
			request: { endpoint, body in
				...
				guard let secretKey: String = try? await keychainService.load(.ollamaSecretKey), !secretKey.isEmpty else {
...
extension OllamaAIClient: DependencyKey {
...
extension DependencyValues {
	var ollamaAIClient: OllamaAIClient {
		get { self[OllamaAIClient.self] }
		set { self[OllamaAIClient.self] = newValue }
	}
}
